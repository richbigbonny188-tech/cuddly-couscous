<?php
/* --------------------------------------------------------------
  ItRechtModuleCenterModuleController.inc.php 2020-06-09
  Gambio GmbH
  http://www.gambio.de
...
class ItRechtModuleCenterModuleController extends AbstractModuleCenterModuleController
{
    ...
    {
        $supported_languages = ['de', 'be', 'fr', 'nl', 'es', 'en', 'sv', 'da', 'it', 'pl'];
        $db                  = StaticGXCoreLoader::getDatabaseQueryBuilder();
        $db->from('languages')->select('languages_id, code')->where('code IN (' . implode(',',
    ...
    protected function _getFiles()
    {
        ...
        $rechtstext_types = ['agb', 'impressum', 'datenschutz', 'widerruf'];
        $files            = [];
        foreach ($rechtstext_types as $rtype) {
            ...
            foreach ($languages as $lang_code => $lang_id) {
                ...
                foreach (['txt', 'html', 'pdf'] as $content_type) {
                    $file_path                                         = sprintf('media/content/itrk_%s_%s.%s',
                                                                                 $rtype,
                                                                                 $lang_code,
                                                                                 $content_type);
                    $full_file_path                                    = DIR_FS_CATALOG . '/' . $file_path;
                    $file_exists                                       = file_exists($full_file_path);
                    $file_date                                         = $file_exists ? date('c',
                                                                                             filemtime($full_file_path)) : 'not received';
    ...
    public function actionDefault()
    {
        ...
        if (!empty($apiToken)) {
            $response = MainFactory::create('RedirectHttpControllerResponse',
                                            xtc_href_link('admin.php', 'do=ItRechtModuleCenterModule/Configuration'));
        } else {
            $response = MainFactory::create('RedirectHttpControllerResponse',
                                            xtc_href_link('admin.php', 'do=ItRechtModuleCenterModule/Info'));
        }
    ...
    public function actionConfiguration()
    {
        ...
        $template      = 'module_center/itrk_configuration.html';
        $subNavigation = [
            [
                'text'   => $this->languageTextManager->get_text('tab_config_page', 'itrecht'),
                'link'   => $formdata['url_config_page'] ?? '',
                'active' => true,
            ],
            [
                'text'   => $this->languageTextManager->get_text('tab_info_page', 'itrecht'),
                'link'   => $formdata['url_info_page'] ?? '',
                'active' => false,
            ],
        ];
    ...
    public function actionInfo()
    {
        ...
        $this->contentView->set_template_dir(DIR_FS_ADMIN . 'html/content/module_center/');
        $template      = 'module_center/itrk_info.html';
        $subNavigation = [
            [
                'text'   => $this->languageTextManager->get_text('tab_config_page', 'itrecht'),
                'link'   => $formdata['url_config_page'] ?? '',
    ...
    public function actionSaveConfiguration()
    {
        ...
        if ($postData->keyExists('gen_token')) {
            ...
            $useWiderrufInPdf = $postData->keyExists('use_withdrawal_in_pdf')
                                && $postData->getValue('use_withdrawal_in_pdf') == true;
            ...
            foreach ($languages as $code => $l_id) {
                ...
                if ($useWiderrufInPdf && $files['widerruf'][$code]['files']['txt']['file_exists'] == true) {
                    $withdrawal_txt = file_get_contents(DIR_FS_CATALOG
                                                        . $files['widerruf'][$code]['files']['txt']['file']);
    ...
    public function actionUseInContentManager()
    {
        ...
        $content_group = array_key_exists($postData->getValue('type'),
                                          $mapping) ? $mapping[$postData->getValue('type')] : false;
        if ($content_group !== false) {
            $db = StaticGXCoreLoader::getDatabaseQueryBuilder();
            $db->set('content_file', $postData->getValue('file'));
            $db->set('content_type', 'file');
            $db->where('content_group', $content_group);
            $db->update('content_manager');
            $GLOBALS['messageStack']->add_session($this->text->get_text('legal_text_copied_to_content_manager'),
                                                  'info');
        } else {
