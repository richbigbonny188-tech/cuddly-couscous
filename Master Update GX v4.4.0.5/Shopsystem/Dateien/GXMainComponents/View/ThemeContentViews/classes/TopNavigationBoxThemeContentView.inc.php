<?php
/* --------------------------------------------------------------
  TopNavigationBoxThemeContentView.inc.php 2020-09-10
  Gambio GmbH
  http://www.gambio.de
...
class TopNavigationBoxThemeContentView extends ThemeContentView
{
    ...
    {
        parent::__construct();
        $this->set_content_template('layout_secondary_navigation.html');
    ...
    public function prepare_data()
    {
        ...
        $this->_setEditCategoryUrl();
        $this->_setEditContentUrl();
    ...
    protected function _setCurrency()
    {
        ...
        if (count($this->coo_xtc_price->currencies) > 1 && $showTopCurrencySelection == 'true'
            && strpos(gm_get_env_info('SCRIPT_NAME'),
                      'checkout') === false) {
    ...
    protected function _setCustomersDataDiscount()
    {
        ...
        if ((double)$_SESSION['customers_status']['customers_status_ot_discount'] > 0
    ...
    protected function _setCustomersDataMinMaxOrder()
    {
        ...
        if ((double)$_SESSION['customers_status']['customers_status_min_order'] > 0) {
            $this->showArrow                       = 1;
            $t_min_order                           = $this->coo_xtc_price->xtcFormat($_SESSION['customers_status']['customers_status_min_order'],
                                                                                     true);
        ...
        if ((double)$_SESSION['customers_status']['customers_status_max_order'] > 0) {
            $this->showArrow                       = 1;
            $t_max_order                           = $this->coo_xtc_price->xtcFormat($_SESSION['customers_status']['customers_status_max_order'],
                                                                                     true);
    ...
    protected function _setCustomersDataIcon()
    {
        ...
        if (file_exists('admin/html/assets/images/legacy/icons/'
                        . basename($_SESSION['customers_status']['customers_status_image']))) {
            $this->customersDataArray['ICON'] = 'admin/html/assets/images/legacy/icons/'
                                                . basename($_SESSION['customers_status']['customers_status_image']);
        }
    }
    ...
    {
        $query = 'SELECT * FROM `languages` WHERE `status` = 1';
        
        $activeLanguages = xtc_db_query($query)->num_rows;
    ...
    {
        /* @var ContentBoxThemeContentView $coo_content */
        $coo_content = MainFactory::create_object('ContentBoxThemeContentView');
        ...
        if (!isset($_SESSION['customer_id']) && gm_get_conf('SHOW_TOP_COUNTRY_SELECTION') === 'true') {
            $coo_countries   = MainFactory::create_object('Countries', [$_SESSION['languages_id'], true, true]);
    ...
    protected function _setAdminUrl()
    {
        ...
        $this->set_content_data('admin_url', 'admin/');
    }
    ...
    protected function _setEditProductUrl()
    {
        if (isset($GLOBALS['actual_products_id'])) {
            ...
            $this->set_content_data('edit_product_url',
                                    'admin/categories.php?cPath=' . $cPath . '&pID=' . $GLOBALS['actual_products_id']
                                    . '&action=new_product');
        }
    }
    
    
    protected function _setEditCategoryUrl()
    {
        if (isset($GLOBALS['cID'])) {
            $cID = (int)$GLOBALS['cID'];
            $this->set_content_data('edit_category_url',
                                    'admin/categories.php?cID=' . $cID . '&action=edit_category');
        }
    }
    
    
    protected function _setEditContentUrl()
    {
        if (isset($_GET['coID'])) {
            $coID              = (int)$_GET['coID'];
            $excludeContentIDs = [3889895];
            if (!in_array($coID, $excludeContentIDs, true)) {
                $this->set_content_data('edit_content_url',
                                        'admin/admin.php?do=ContentManagerPages/edit&id=' . $coID);
            }
        }
    }
    ...
    public function set_top_navigation_template()
    {
        $this->set_content_template('layout_secondary_navigation.html');
    }
}
