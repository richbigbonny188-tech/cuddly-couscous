...
function RequestHandler()
{
	...
	this.check_for_updates = function()
	{
		...
		$.ajax({
			...
			success:	function(p_response)
			{
				coo_this.updatesAvailable = p_response['updates_available'];
		...
		$.ajax(
		{
			...
			success:	function(p_response)
			{
				if (p_response !== 'null') {
					...
					$('#admin_cache_btn').css('display', 'inline-block');
				} else {
					$('#clear_cache').hide();
					$('#installation_success').show();
					$('#admin_store_btn').css('display', 'inline-block');
					
					if(coo_this.updatesAvailable){
						$('.auto-updater-update-available').show();
					}
				}
			},
			error:	function(p_response)
			{
				$('#clear_cache').hide();
				$('#installation_success_cache_error').show();
				$('#result .btn').css('display', 'inline-block');
			}
		});
	}
}
