<?php
/*--------------------------------------------------------------------
 set_shipping_status.php 2020-11-04
 Gambio GmbH
 http://www.gambio.de
 ...
 * @return bool true:OK
 */
function set_shipping_status($p_product_id, $p_combi_id = false, $p_product_old_quantity = false)
{
	...
	{
		if($p_product_old_quantity != false && $c_product_quantity > $p_product_old_quantity)
        {
            $t_query = 'SELECT
						shipping_status_id
					FROM
		...
		if(xtc_db_num_rows($t_new_shipping_status_query) > 0)
		{
            $t_shipping_status_id_new = [];
            while($t_new_shipping_status_values = xtc_db_fetch_array($t_new_shipping_status_query))
            {
                $t_shipping_status_id_new[] = $t_new_shipping_status_values['shipping_status_id'];
            }
            $t_shipping_status_id_new = $t_shipping_status_id_new[0];
