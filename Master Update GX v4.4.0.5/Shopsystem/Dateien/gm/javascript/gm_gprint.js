...
$(document).ready(function()
{
	...
	elseif(!defined('GM_GPRINT_ADMIN'))
	{
		...
		if(!empty($this->v_data_array['GET']['id']))
		{
			...
			var t_gprint_position = '<?= gm_get_conf('CUSTOMIZER_POSITION'); ?>';
	...
	{	
        ...
        if(c_surfaces_group_name != '' || c_surfaces_groups_id > 0)
        {
        	var t_surfaces_groups_id = jQuery.ajax({
	            data: 'action=copy_surfaces_group&name=' + c_surfaces_group_name + '&surfaces_groups_id=' + c_surfaces_groups_id + '&mode=' + c_mode + '&page_token=' + page_token,
	...
	$('#create_surfaces_group').click(function()
	{
		...
		if (f_surfaces_group_name !== '') {
			$('#surfaces_group_name').removeClass('error');
			...
			$('#surfaces_group_name').addClass('error');
		}
    });
