<?php
/* --------------------------------------------------------------
	TrustedShopsProductInfoThemeContentView.inc.php 2020-12-23
	Gambio GmbH
	http://www.gambio.de
...
class TrustedShopsProductInfoThemeContentView extends TrustedShopsProductInfoThemeContentView_parent
{
    ...
    protected function _assignDescription()
    {
        ...
        if ((bool)gm_get_conf('MODULE_CENTER_TRUSTEDSHOPS_INSTALLED') === true) {
            $service = MainFactory::create_object('GMTSService');
            if ((bool)$service->productreviews_enabled === true) {
                $tsid = $service->findRatingID($_SESSION['language_code']);
                if ($tsid !== false) {
                    $this->content_array['tabs'][] = [
                        'title' => $service->get_text('product_reviews_tab_title'),
                        'content' => $this->_getReviewSnippet($tsid),
                    ];
                }
            }
        }
    ...
    protected function _getSKUForTrustedShops()
    {
        ...
        if (!empty($this->product->data['products_model'])) {
            ...
            if (!empty($this->product->data['products_ean'])) {
