<?php
/* --------------------------------------------------------------
   CreateAccountContentView.inc.php 2019-09-12
   Gambio GmbH
   http://www.gambio.de
...
class CreateAccountContentView extends ContentView
{
    ...
    {
        $t_form_action = xtc_href_link('shop.php', 'do=CreateRegistree/Proceed&' . $this->checkout_started_param,
            'SSL');
        if ($this->guest_account) {
            $t_form_action = xtc_href_link('shop.php', 'do=CreateGuest/Proceed&' . $this->checkout_started_param,
                'SSL');
        }
    ...
    protected function add_birthdate()
    {
        if (ACCOUNT_DOB == 'true') {
            ...
            $this->content_array['form_data']['birthdate']['default_value'] = date('01.01.Y',
                strtotime(date('Y-01-01') . ' -10 years'));
    ...
    protected function add_state()
    {
        ...
        $countryId = $this->customer_data_array['country']
    ...
    {
        $this->set_content_template('module/create_account_guest.html');
    }
}
