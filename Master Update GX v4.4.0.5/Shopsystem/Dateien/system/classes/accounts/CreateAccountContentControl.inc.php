<?php
/* --------------------------------------------------------------
   CreateAccountContentControl.inc.php 2020-05-12
   Gambio GmbH
   http://www.gambio.de
...
class CreateAccountContentControl extends DataProcessing
{
    ...
    protected function get_customers_data()
    {
        ...
        $isStateMandatory = $coo_country_service->isStateMandatory(new IdType($this->v_data_array['POST']['country']));
        $country = $coo_country_service->getCountryById(new IdType($this->v_data_array['POST']['country']));
        ...
        // vat_validation
        $coo_vat_validation = new vat_validation($this->customer_data_array['vat'], '', '',
            $this->customer_data_array['country'], $this->guest_account);
    ...
    {
        if (ACCOUNT_GENDER == 'true') {
            if (($this->customer_data_array['gender'] != 'm') && ($this->customer_data_array['gender'] != 'f') && ((string)$this->customer_data_array['gender'] === '' && GENDER_MANDATORY === 'true')) {
                $this->error = true;
        ...
        if (ACCOUNT_DOB == 'true' && (ENTRY_DOB_MIN_LENGTH > 0 || ($this->customer_data_array['dob'] != '' && ENTRY_DOB_MIN_LENGTH == 0))) {
            if (!preg_match('/^[0-9]{2}[\.\/]{1}[0-9]{2}[\.\/]{1}[0-9]{4}$/', $this->customer_data_array['dob']) ||
                checkdate(substr(xtc_date_raw($this->customer_data_array['dob']), 4, 2),
                    substr(xtc_date_raw($this->customer_data_array['dob']), 6, 2),
                    substr(xtc_date_raw($this->customer_data_array['dob']), 0, 4)) == false) {
                $this->error = true;
    ...
    protected function save_address_book()
    {
        ...
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS, $t_customers_array, 'update',
            'customers_id = ' . (int)$this->customer_data_array['customer_id']);
    }
    ...
    protected function create_mail_data_array()
    {
        ...
        $this->mail_data_array['logo_path'] = HTTP_SERVER . DIR_WS_CATALOG . StaticGXCoreLoader::getThemeControl()->getThemeImagePath();
        ...
        if ($this->gift_array != false) {
            ...
            $this->mail_data_array['GIFT_LINK'] = xtc_href_link(FILENAME_GV_REDEEM,
                'gv_no=' . $this->gift_array['coupon_code'], 'NONSSL', false);
        }
    ...
    {
        if ($this->guest_account) {
            $this->coo_create_account_content_view->set_register_guest_template();
        }
    ...
    public function track($p_customer_id)
    {
        if (isset($_SESSION['tracking']['refID'])) {
            ...
            $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS, array('refferers_id' => $refID), 'update',
                'customers_id = ' . (int)$p_customer_id);

            $leads = $campaign['campaigns_leads'] + 1;
            $this->wrapped_db_perform(__FUNCTION__, TABLE_CAMPAIGNS, array('campaigns_leads' => $leads), 'update',
                'campaigns_id = ' . $refID);
        }
    }
    ...
    public function delete_account($p_customers_id)
    {
        ...
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS, array(), 'delete',
            'customers_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_ADDRESS_BOOK, array(), 'delete',
            'customers_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS_INFO, array(), 'delete',
            'customers_info_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS_BASKET, array(), 'delete',
            'customers_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS_BASKET_ATTRIBUTES, array(), 'delete',
            'customers_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS_IP, array(), 'delete',
            'customers_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS_WISHLIST, array(), 'delete',
            'customers_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS_WISHLIST_ATTRIBUTES, array(), 'delete',
            'customers_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_CUSTOMERS_STATUS_HISTORY, array(), 'delete',
            'customers_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_COUPON_GV_CUSTOMER, array(), 'delete',
            'customer_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_COUPON_GV_QUEUE, array(), 'delete',
            'customer_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_COUPON_REDEEM_TRACK, array(), 'delete',
            'customer_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, TABLE_WHOS_ONLINE, array(), 'delete',
            'customer_id = ' . $c_customers_id);
        $this->wrapped_db_perform(__FUNCTION__, 'withdrawals', array('customer_id' => null), 'update',
            'customer_id = ' . $c_customers_id);
    }
}
