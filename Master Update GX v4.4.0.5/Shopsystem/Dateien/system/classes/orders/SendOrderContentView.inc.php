<?php
/* --------------------------------------------------------------
   SendOrderContentView.inc.php 2020-12-01
   Gambio GmbH
   http://www.gambio.de
...
class SendOrderContentView extends ContentView
{
    ...
    protected function set_validation_rules()
    {
        ...
        $this->validation_rules_array['coo_order'] = array(
            'type' => 'object',
            'object_type' => 'order'
        );
    }
    ...
    public function get_mail_content_array()
    {
        ...
        if (extension_loaded('intl')) {
            $t_language_provider = MainFactory::create(
            ...
            $t_date_purchased = new DateTime($this->order->info['date_purchased']);
            $this->set_content_data(
                'DATE',
                utf8_encode_wrapper(DateFormatter::formatAsFullDate($t_date_purchased, $t_language_code))
            );
        } else {
            $this->set_content_data(
                'DATE',
        ...
        if ($this->payment_module === 'cod') {
            $t_cod_info = get_transfer_charge_text($this->shipping_module, $t_order->delivery['country_iso_2'],
                $this->order->customer['status'], $t_order->info['currency']);
        ...
        $this->set_content_data('address_label_customer',
            xtc_address_format($t_order->customer['format_id'], $t_order->customer, 1, '', '<br />'));
        $this->set_content_data('address_label_shipping',
            xtc_address_format($t_order->delivery['format_id'], $t_order->delivery, 1, '', '<br />'));
        if ($this->credit_covers != '1') {
            $this->set_content_data('address_label_payment',
                xtc_address_format($t_order->billing['format_id'], $t_order->billing, 1, '', '<br />'));
        }

        // GET HTML MAIL CONTENT
        $t_html_output_array['html'] = fetch_email_template($this, 'order_mail', 'html', '', $t_language_id,
            $t_language);
        ...
        $t_html_output_array['txt'] = strip_tags(html_entity_decode_wrapper(fetch_email_template($this, 'order_mail', 'txt', '', $t_language_id,
            $t_language)));
