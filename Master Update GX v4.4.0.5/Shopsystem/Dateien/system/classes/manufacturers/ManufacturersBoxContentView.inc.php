<?php
/* --------------------------------------------------------------
  ManufacturersBoxContentView.inc.php 2019-04-16
  Gambio GmbH
  http://www.gambio.de
...
class ManufacturersBoxContentView extends ContentView
{
    ...
    protected function add_manufacturer_list()
    {
        ...
        foreach ($this->manufacturer_array as $t_manufacturer) {
            $t_manufacturers_name = ((strlen_wrapper($t_manufacturer['manufacturers_name']) > MAX_DISPLAY_MANUFACTURER_NAME_LEN) ? substr_wrapper($t_manufacturer['manufacturers_name'],
                    0, MAX_DISPLAY_MANUFACTURER_NAME_LEN) . '...' : $t_manufacturer['manufacturers_name']);
            ...
            $this->content_array['CONTENT'] .= '<a href="' . xtc_href_link(FILENAME_DEFAULT,
                    'manufacturers_id=' . $t_manufacturer['manufacturers_id']) . '">' . $t_manufacturers_name . '</a><br />';
        }
    }
    ...
    protected function add_manufacturer_drop_down()
    {
        ...
        foreach ($this->manufacturer_array as $t_manufacturer) {
            $t_manufacturers_name = ((strlen_wrapper($t_manufacturer['manufacturers_name']) > MAX_DISPLAY_MANUFACTURER_NAME_LEN) ? substr_wrapper($t_manufacturer['manufacturers_name'],
                    0, MAX_DISPLAY_MANUFACTURER_NAME_LEN) . '..' : $t_manufacturer['manufacturers_name']);
            $t_manufacturers_array[] = array(
                'id' => $t_manufacturer['manufacturers_id'],
                'text' => $t_manufacturers_name
            );
        }

        $this->content_array['CONTENT'] = xtc_draw_form('manufacturers',
                xtc_href_link(FILENAME_DEFAULT, '', 'NONSSL', false, true, false),
                'get') . xtc_draw_pull_down_menu('manufacturers_id', $t_manufacturers_array, $this->manufacturer_id,
                'onchange="if(this.value!=\'\'){this.form.submit();}" size="' . MAX_MANUFACTURERS_LIST . '" class="lightbox_visibility_hidden input-select"') . '</form>';
    }
}
