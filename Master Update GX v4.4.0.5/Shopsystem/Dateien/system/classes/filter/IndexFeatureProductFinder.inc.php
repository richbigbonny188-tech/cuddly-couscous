<?php
/*--------------------------------------------------------------------
 IndexFeatureProductFinder.inc.php 2021-03-30
 Gambio GmbH
 http://www.gambio.de
...
class IndexFeatureProductFinder
{
	...
	function get_products_listing_sql_string($p_where_part = '', $p_order_by = '')
	{
		...
		if(isset($GLOBALS['coo_debugger']) && is_object($GLOBALS['coo_debugger'])) $GLOBALS['coo_debugger']->log('get_products_listing_sql_string() SQL: '. $t_output_sql, 'IndexFeatureProductFinder');
		return $t_output_sql;
	}
