	...
	class order_ORIGIN
	{
		...
		function query($order_id)
		{
			...
			$this->info = array('orders_id' => $order['orders_id'],
								...
								'shipping_method' => ((substr($shipping_method['title'], -1) == ':') ? substr($shipping_method['title'], 0, -1) : $shipping_method['title']),
			...
			$this->info['shipping_method'] = strip_tags(str_replace('<br />', '&nbsp;', $this->info['shipping_method']));
		...
		function cart()
		{
			...
			$this->info = array('order_status' => DEFAULT_ORDERS_STATUS_ID,
								...
								'shipping_class' => $_SESSION['shipping']['id'] ?? null,
								'payment_class' => $_SESSION['payment'] ?? null);
            ...
            $scriptName = basename(gm_get_env_info('SCRIPT_NAME'));
            if (isset($_SESSION['customer_country_id'], $_SESSION['customer_zone_id']) && strpos($scriptName, 'checkout') !== 0) {
                /* outside checkout, use delivery country from dropdown/popup in cart */
                ...
                $this->delivery = [
                    ...
                    'country'                 => [
                        'id'         => $customerCountry->getId(),
                        'title'      => $customerCountry->getName()->asString(),
                        'iso_code_2' => (string)$customerCountry->getIso2(),
                        'iso_code_3' => (string)$customerCountry->getIso3(),
                    ],
                    'country_id'              => $customerCountry->getId(),
                    'format_id'               => $customerCountry->getAddressFormatId()->asInt(),
                ];
                $tax_address['entry_country_id'] = $customerCountry->getId();
                $tax_address['entry_zone_id'] = $_SESSION['customer_zone_id'];
            } else {
                $this->delivery = [
                    'firstname'               => $shipping_address['entry_firstname'],
                    'lastname'                => $shipping_address['entry_lastname'],
			...
			for($i = 0, $n = sizeof($products); $i < $n; $i++)
			{
                ...
                $taxCountry         = $selfpickupSelected ? $xtPrice->getShippingOriginCountryAndZoneId()['country_id'] : $tax_address['entry_country_id'];
                $taxZone            = $selfpickupSelected ? $xtPrice->getShippingOriginCountryAndZoneId()['zone_id'] : $tax_address['entry_zone_id'];
                ...
                $this->products[$index] = array('qty' => (double)$products[$i]['quantity'],
												...
												'tax' => xtc_get_tax_rate($products[$i]['tax_class_id'], $taxCountry, $taxZone),
												'tax_description' => xtc_get_tax_description($products[$i]['tax_class_id'], $taxCountry, $taxZone),
				...
				$_SESSION['customer_cart_tax_info'] = [
					$this->products[$index]['tax_description'] => [
						'tax_class_id' => $products[$i]['tax_class_id'],
						'country_id'   => $tax_address['entry_country_id'],
						'zone_id'      => $tax_address['entry_zone_id']
					]
				];
                ...
                $this->info['subtotal'] = $this->info['subtotal'] ?? 0;
                ...
                $this->info['tax'] = $this->info['tax'] ?? 0;
                
				if($_SESSION['customers_status']['customers_status_show_price_tax'] == '1')
				{
                    $this->info['tax_groups'] = $this->info['tax_groups'] ?? [TAX_ADD_TAX . "$products_tax_description" => 0];
				...
				{
                    $this->info['tax_groups'] = $this->info['tax_groups'] ?? [TAX_NO_TAX . "$products_tax_description" => 0];
