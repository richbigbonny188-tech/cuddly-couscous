<?php
/* --------------------------------------------------------------
   class.inputfilter.php 2022-01-24
   Gambio GmbH
   http://www.gambio.de
   Copyright (c) 2022 Gambio Gambio
...
class InputFilter_ORIGIN {
	...
	function process($source, $get=false, $t_exclude_array = array()) {
        ...
        if (is_array($source)) { 
            foreach ($source as $key => $value) { 
				...
				if (defined('SEARCH_ENGINE_FRIENDLY_URLS') && SEARCH_ENGINE_FRIENDLY_URLS === 'true' && $get == true) {
                ...
                if ($key == $tmp_key) {
					if (in_array($key, $this->keyWhitelist, true)) {
	...
	function filterTags($source) {
		...
		while ($source !== str_replace('<>', '', $source)) {
