 ...
 * @version $Id: product.php 11115 2025-05-05 12:59:47Z Milbo $
 */
...
class VirtuemartControllerProduct extends VmController {
	...
	static function moveUploadedFileAndXRef($virtuemart_product_id, $file, &$ret){
		...
		$isImage = null;
		$NewImageName = VmUploader::checkUploadFile(self::$output_dir, $file, false, $isImage);
		if(!empty($NewImageName)){
			$file_title = $file['name'];
			$file_type = $file['type'];
			$mediaModel = VmModel::getModel('media');
			$img_file_path = self::$imagepath. $NewImageName;
			$mediaUpload = $mediaModel->saveMultiImageMedia($virtuemart_product_id, $file_title, $img_file_path, $file_type, 'product', $isImage);
			$ret['$NewImageName'] =  self::$output_dir.$NewImageName;
		} else {
			$ret['$NewImageName'] =  'error';
		}
