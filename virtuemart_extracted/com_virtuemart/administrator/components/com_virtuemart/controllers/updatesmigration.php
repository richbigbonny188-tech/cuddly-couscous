 ...
 * @version $Id: updatesmigration.php 11107 2025-02-03 11:27:26Z Milbo $
 */
...
class VirtuemartControllerUpdatesMigration extends VmController{
	...
	/**
	 * loads all media and checks if image or not and sets the is_image field to the table
	 * @return void
	 */
	function setIsImage(){
		//Load all media with is_image = 0 or is_image is null
		if(!vmAccess::manager()){
			VmWarn('COM_VIRTUEMART_ACCESS_DENIED');
			return false;
		}

		$db = JFactory::getDBO();
		$q = 'SELECT `virtuemart_media_id`,`is_image`,`file_url` FROM `#__virtuemart_medias` WHERE `is_image` = 0 OR `is_image` IS NULL';
		$db->setQuery($q);
		$medias = $db->loadObjectList();

		//$medias = $model->getFiles(false, true, null, null, array('is_image = 0','is_image = null'));
		$mediasSet = array();
		foreach($medias as $media){

			$is_image = VmMediaHandler::isImage($media->file_url);
			if($is_image!=$media->is_image){

				$q = 'UPDATE `#__virtuemart_medias` SET `is_image` = "'.$is_image.'" WHERE `virtuemart_media_id` = "'.$media->virtuemart_media_id.'" ';
				$db->setQuery($q);
				$db->execute();
				$mediasSet[] = $media->virtuemart_media_id;
			}
		}

		vmInfo('Set is_image for media ids '.implode(', ', $mediasSet));
		$this->setRedirect($this->redirectPath);
	}
