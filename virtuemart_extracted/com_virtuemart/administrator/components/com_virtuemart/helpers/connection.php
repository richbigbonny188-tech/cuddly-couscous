	...
	static function getHttp($options = [], $adapters = null){
		...
		if(JVM_VERSION < 4){
			...
			if ($options === null) {
				...
				if (!empty($options) and method_exists($options, 'toArray')) {
					$options = $options->toArray();
				} else {
					if (empty($options)) {
						throw new \InvalidArgumentException(
							'VmConnector::getHttp The options param must be not empty.'
						);
					} else {
						throw new \InvalidArgumentException(
							'VmConnector::getHttp The options param must be an array or implement the ArrayAccess interface.'
						);
					}
